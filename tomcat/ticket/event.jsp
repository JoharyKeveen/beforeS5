...
<%
    ...
    Vector<Evenement> list=new Evenement().get_Tab(new Evenement().select(lo));
...
<div style="text-align: center;">
    <h1>Veuillez choisir un evenement :</h1>
    <%
        for(int i=0;i<list.size();i++)
        {
            %>
            <h3>
                <a href="emplacement.jsp?id=<% out.print(list.get(i).getId()); %>&&client=<% out.print(client); %>" style="text-decoration: none;"><% out.print(list.get(i).getNom()); %> <% out.print(list.get(i).getDaty()); %></a>
            </h3>
            <%
        }
        lo.close();
    %>
    <h3>
        <a href="reservation.jsp?client=<% out.print(client); %>">Voir mes reservations</a>
    </h3>
</div>
</body>
</html>